...
public class SqliteTimerServiceTests : IDisposable
{
	private readonly string _testDbPath;
	private readonly SqliteTimerService _service;

	public SqliteTimerServiceTests()
	{
		// Create a unique test database for each test run
		_testDbPath = $"test_timers_{Guid.NewGuid()}.db";
		_service = new SqliteTimerService(_testDbPath);
	}
	...
	public void Dispose()
	{
		...
		// Try to delete the test database file, retry if locked
		if (File.Exists(_testDbPath))
		{
			for (int i = 0; i < 10; i++)
			{
				...
				catch (IOException)
				{
					if (i == 9)
					{
						// If we still can't delete after 10 retries, just ignore it
